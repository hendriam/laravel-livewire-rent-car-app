<div class="space-y-4">
    <h2 class="text-xl font-bold">Manajemen Supir</h2>
    ...
    <x-ui.admin.card>
        ...
        <x-ui.admin.card-header>
            <div class="w-full md:w-1/2">
                <x-ui.input-search placeholder="Cari nama supir..." />
            </div>
        ...
        <x-ui.admin.card-body>
            <x-ui.table :data="$drivers" :columns="[
            ...
            >
                @foreach($drivers as $driver)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
                        <th class="px-4 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ $driver->fullname }}</th>
                        <td class="px-4 py-2 ">{{ $driver->phone }}</td>
                        <td class="px-4 py-2 ">{{ $driver->address }}</td>
                        <td class="px-4 py-2 ">{{ $driver->status }}</td>
                        <td class="px-4 py-2 ">{{ $driver->createdBy->fullname }}</td>
                        <td class="px-4 py-2 ">{{ $driver->created_at ?? ''  }}</td>
                        <td class="px-4 py-2 ">
                            <div class="flex space-x-1">
                                <a href="{{ route('admin.drivers.edit', $driver->id) }}" class="">
                                ...
                                <x-ui.admin.button-red
                                    type="button"
                                    wire:click="$set('idToDelete', {{ $driver->id }})"
        ...
        <x-ui.admin.card-footer>
            <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
                Showing
                <span class="font-semibold text-gray-900 dark:text-white">{{ $drivers->firstItem() }} - {{ $drivers->lastItem() }}</span>
                of
                <span class="font-semibold text-gray-900 dark:text-white">{{ $drivers->total() }}</span>
            </span>
            <x-ui.admin.pagination :paginator="$drivers" />
        </x-ui.admin.card-footer>
    </x-ui.admin.card>
</div>